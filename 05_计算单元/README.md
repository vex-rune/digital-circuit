...
# 指令集的定义
| 指令区间 | 占用位数 | 字段名 | 编码规则 & 功能 |
|------|------|------|------|
| bit0～bit1 | 2 位 | ALU_OP 运算码 | 00：A+B 加法01：A−B 减法10：A&B 按位与11：A|B 按位或 |
| bit2～bit3 | 2 位 | SEL_A 停机控制 | 00：停止指令，程序停机01/10/11：保留，无停机作用 |
| bit4～bit5 | 2 位 | SEL_B 操作数选择 | 00：切换输入，选 A / 选 B01/10/11：保留，不切换数据源 |
| bit6～bit10 | 5 位 | RESERVED 保留位 | 未定义功能，硬件默认填 0，可后续扩展指令 |
| bit11 | 1 位 | STR 存储使能 | 1：开启存储，结果写入内存 / 寄存器0：不保存结果 |
| bit12 | 1 位 | LD 输出加载 | 1：向外输出运算结果0：关闭输出 |
| bit13～bit14 | 2 位 | RESERVED 保留位 | 未定义，默认 0，预留扩展控制信号 |
| bit15 | 1 位 | HALF 中断运行位 | 1：程序正常继续执行0：触发中断，暂停全部运算流程 |
